...
def test_factor_embedding_basic(mock_adata):
    ...
    ax = factor_embedding(mock_adata, "pca", factors=["0", 1], basis="X_umap")
...
def test_factor_embedding_single_factor(mock_adata):
    ...
    ax = factor_embedding(mock_adata, "pca", factors=["0"], basis="X_umap")
...
def test_factor_embedding_custom_basis(mock_adata):
    ...
    ax = factor_embedding(mock_adata, "pca", factors=0, basis="X_custom")
...
def test_factor_embedding_custom_cmap(mock_adata):
    ...
    ax = factor_embedding(mock_adata, "pca", factors=0, basis="X_umap", cmap="viridis")
...
def test_factor_embedding_custom_size(mock_adata):
    ...
    size = 5
    ax = factor_embedding(mock_adata, "pca", factors=["0"], basis="X_umap", size=size)
...
def test_factor_embedding_custom_annotation(mock_adata):
    ...
    ax = factor_embedding(
        mock_adata, "pca", factors=["0"], basis="X_umap", annotation=custom_key
    )
...
def test_factor_embedding_invalid_basis(mock_adata):
    ...
    with pytest.raises(KeyError):
        factor_embedding(mock_adata, "pca", factors=0, basis="invalid_basis")


def test_factor_embedding_invalid_factor(mock_adata):
    """Test factor embedding with invalid factor."""
    with pytest.raises(IndexError):
        factor_embedding(mock_adata, "pca", factors=10, basis="X_umap")
...
def test_factor_embedding_custom_ax(mock_adata):
    ...
    result_ax = factor_embedding(
        mock_adata, "pca", factors=["0"], basis="X_umap", ax=ax
    )
